...
# This function is going to keep running and fetch data from the arduino
    global sig1, sig2, sig3, sig4
    while not breaking_case:
        received = False
        while not received:
            try:
                ser_out = ser.readline().decode().strip().split(',')
                if not len(ser_out) == 4:
                    continue
                else:
                    received = True
                    sig4 = float(ser_out[0])
                    sig3 = float(ser_out[1])
                    sig2 = float(ser_out[2])
                    sig1 = float(ser_out[3])
...
while True:
    print(f"left: {sig1}, right: {sig2} left: {sig3}, right: {sig4}")
    for event in pygame.event.get():
        # Exit game
        if event.type == pygame.QUIT:
            pygame.quit()
    ...
    if ARDUINO_MODE:
        ...
        if cam_y%10 == 0:
            ...
            if sig3 > 0 and sig4 > 0:
                print("forward")
                player_2.move_y(-player_speed)
            elif sig3 > 0:
                print("left")
                player_2.move_x(-player_speed)
            elif sig4 > 0:
                print("right")
                player_2.move_x(player_speed)
